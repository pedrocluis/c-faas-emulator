...
CONTAINERS * initPodman(int n_threads) {
    ...
    for (int i = 0; i < n_threads; i++) {
        containers->thread_ids[sizeof (pid_t) * i] = 0;
        containers->curl_handles[sizeof (CURL *) * i] = curl_easy_init();
    }
...
void destroyPodman(CONTAINERS *containers, int n_threads) {
    ...
    for (int i = 0; i < n_threads; i++) {
        curl_easy_cleanup(containers->curl_handles[sizeof (CURL *) * i]);
    }
...
char* createContainer(CONTAINERS *containers, invocation_t *invocation) {
    ...
    memset(cmd, 0, 200);
    sprintf(cmd, "podman create --privileged -p %d:8080/tcp docker.io/openwhisk/action-python-v3.9", port);
    FILE *fp;
    ...
    fgets(containerId, sizeof(containerId), fp);
    /* close */
    pclose(fp);
    free(cmd);
...
void initFunction(int port, CONTAINERS *containers, int tid) {
    ...
    CURL *curl = containers->curl_handles[tid * sizeof (CURL*)];
    CURLcode res;
